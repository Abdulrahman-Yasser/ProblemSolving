...
void initDistance(int distance[MAX_CITIES_CNT][MAX_CITIES_CNT], int src, int dest, int val, int cityCnt, int isThereRoad[MAX_CITIES_CNT][MAX_CITIES_CNT]){
    distance[src][dest] = val;
    for(int nextDest = 0; nextDest < cityCnt; nextDest++){
        if(isThereRoad[dest][nextDest] && distance[src][nextDest] == -1){
            initDistance(distance, src, nextDest, val+1, cityCnt, isThereRoad);
        }
    }
}

int numberOfWays(int roads_rows, int roads_columns, int roads[][roads_columns]){
    int RoadExist[MAX_CITIES_CNT][MAX_CITIES_CNT] = {0};
    int cityCnt = roads_rows + 1;
    for(int i = 0; i < roads_rows; i++){
        RoadExist[roads[i][0] - 1][roads[i][1] - 1] = 1;
        RoadExist[roads[i][1] - 1][roads[i][0] - 1] = 1;
    }
    ...
    printDistance(cityCnt, 2, distance);
    for(int src = 0; src < cityCnt; src++){
        initDistance(distance, src, src, 0, cityCnt, RoadExist);
    }
    printDistance(cityCnt, 2, distance);
