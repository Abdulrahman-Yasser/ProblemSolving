...
void count_sort(std::vector<std::string> &my_array)
{
    ...
    std::vector<std::string> *freq_arr = new std::vector<std::string>[26];
    int c = 0;
    int j;
    for (auto i = my_array.begin(); i < my_array.end(); i++)
    {
        freq_arr[ i[0][0] - 'a' ].push_back(*i);
    }
...
int main()
{
    std::vector<std::string> the_array = {"ana", "mohamed", "hossam", "ahmed", "kasem", "ali"};
